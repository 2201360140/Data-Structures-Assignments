# Assignment 47: Dijkstra’s Algorithm Implementation
...
Key components:
- **Adjacency Matrix**: A 2D array where the entry at row i and column j represents the weight of the edge from node i to node j. If no edge exists, the value is 0 or infinity.
...
void dijkstra(const vector<vector<int>> &adj, int n, int src, vector<int> &dist, vector<int> &parent)
{
...
int main()
{
    ...
    vector<vector<int>> adj(n, vector<int>(n, INT_MAX)); // Adjacency Matrix
...
PS D:\Kanchan_study_material\DS\DS_Labs\output> & .\'ASS47.exe'
...
## Conclusion
Dijkstra’s algorithm successfully computes the shortest path in a graph with non-negative edge weights using an adjacency matrix representation. The implementation demonstrates the use of a priority queue for efficiency. The program handles user input for graph construction and path queries, providing clear output for the adjacency matrix and shortest distances. This approach is suitable for dense graphs but may be less efficient for sparse graphs compared to adjacency list representations. Overall, the algorithm is fundamental in network routing and pathfinding applications.
